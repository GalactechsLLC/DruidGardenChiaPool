...
@WebServlet( value = { "/api/rest/v1/poolnetspacetimeline/*", "/api/rest/v1/poolnetspacetimeline" })
@RestPattern( uri = "/api/rest/v1/poolnetspacetimeline/?{start}?/?{end}?")
public class PoolNetspaceTimeline extends RestEndpoint{
	...
	protected ArrayList<Dataset<Long, Long>> loadPoolNetspaceTimeline(@UriVariable(name="start") String start, @UriVariable(name="end") String end) {
		long st, ed;
		try {
		...
		return PoolFeaturesDAO.getPoolNetspaceTimeline(st, ed);
	}
}
