...
public class FarmerPutPayload implements Streamable {
	...
	public FarmerPutPayload(Bytes32 launcher_id, NativeUInt64 authentication_token, Bytes48 authentication_public_key, String payout_instructions, NativeUInt64 suggested_difficulty) {
		this.launcherId = launcher_id;
		this.authenticationPublicKey = authentication_public_key;
		this.payoutInstructions = payout_instructions;
		this.authenticationToken = authentication_token;
		this.suggestedDifficulty = suggested_difficulty;
	}

	public FarmerPutPayload() {
	}
	...
	public Bytes32 hash() {
		...
		if (this.getAuthenticationPublicKey() != null) {
			byteBuf.put((byte) 1);
			byteBuf.put(this.getAuthenticationPublicKey().getBytes());
		} else {
			byteBuf.put((byte) 0);
		}
		if (this.getPayoutInstructions() != null) {
			byteBuf.put((byte) 1);
